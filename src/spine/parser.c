...
static spASTNode* parse_declaration(spTokenList list, size_t* current, const char* filename) {
    // gets type
...
static spASTNode* parse_assign(spTokenList list, size_t* current, const char* filename) {
...
static spASTNode* parse_statement(spTokenList list, size_t* current, const char* filename) {
...
spProgramNode* parse(spTokenList list, const char* filename) {
    ...
    while (list.tokens[current].type != SP_TOKEN_TYPE_EOF) {
        spASTNode* stmt = parse_statement(list, &current, filename);
